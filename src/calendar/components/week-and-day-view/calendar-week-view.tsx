...
import { CustomAddEventDialog } from "@/calendar/components/dialogs/custom-add-event-dialog";
...
export function CalendarWeekView({ singleDayEvents, multiDayEvents }: IProps) {
  ...
  const { services, workers, customers, onCreateAppointment, onQuickCreateCustomer } = useAppointmentContext();
  ...
  return (
    <>
      <div className="flex flex-col items-center justify-center border-b py-4 text-sm text-muted-foreground sm:hidden">
        <p>{t.weeklyViewNotAvailable}</p>
        <p>{t.switchToDailyOrMonthly}</p>
      </div>
            ...
            <div className="grid flex-1 grid-cols-7 divide-x border-l">
              {weekDays.map((day, index) => (
                <span key={index} className="py-2 text-center text-xs font-medium text-muted-foreground">
                  {format(day, "EE", { locale: dateFnsLocale })} <span className="ml-1 font-semibold text-foreground">{format(day, "d", { locale: dateFnsLocale })}</span>
                </span>
              ))}
            </div>
          </div>
              ...
              {hours.map((hour, index) => (
                <div key={hour} className="relative" style={{ height: "96px" }}>
                  <div className="absolute -top-3 right-2 flex h-6 items-center">
                    {index !== 0 && <span className="text-xs text-muted-foreground">{format(new Date().setHours(hour, 0, 0, 0), "hh a", { locale: dateFnsLocale })}</span>}
                  </div>
                </div>
              ))}
            </div>
