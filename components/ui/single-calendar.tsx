...
import { buttonVariants } from "@/components/ported/ui/button";
...
function SingleCalendar({ className, classNames, showOutsideDays = true, selected, ...props }: DayPickerSingleProps & { className?: string; classNames?: any; showOutsideDays?: boolean }) {
  ...
  return (
    <DayPicker
      ...
      components={{
        IconLeft: ({ className, ...props }: { className?: string; [key: string]: any }) => <ChevronLeft className={cn("h-4 w-4", className)} {...props} />,
        IconRight: ({ className, ...props }: { className?: string; [key: string]: any }) => <ChevronRight className={cn("h-4 w-4", className)} {...props} />,
      } as any}
      {...props}
    />
  );
