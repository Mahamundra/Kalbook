...
>(({ className, children, dir, ...props }, ref) => {
  ...
  const triggerDir = dir || (isRTL ? 'rtl' : 'ltr');
...
>(({ className, children, position = "popper", dir, ...props }, ref) => {
  const { isRTL } = useDirection();
  const contentDir = dir || (isRTL ? 'rtl' : 'ltr');
  ...
  return (
    <SelectPrimitive.Portal>
      <SelectPrimitive.Content
        ...
        position={position}
        style={isRTL ? { direction: 'rtl', flexDirection: 'row' } : { direction: 'ltr', flexDirection: 'row' }}
        {...props}
      >
...
>(({ className, children, ...props }, ref) => {
  ...
  return (
    <SelectPrimitive.Item
      ref={ref}
      className={cn(
        "relative flex w-full cursor-default select-none items-center rounded-sm py-1.5 text-sm outline-none data-[disabled]:pointer-events-none data-[disabled]:opacity-50 data-[state=checked]:bg-[#ff3e1b] data-[state=checked]:text-white focus:bg-[#ff3e1b] focus:text-white",
